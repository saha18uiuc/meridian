 ...
 *
 * The Batch Number belongs to the first list even though the SOP's four-identifier sentence does
 * not mention it, because two other statements in the specification require it: the commercial
 * invoice input marks `batchNumber` required, and the certificate rule reads "every batch named on
 * an invoice must be covered by exactly one certificate of analysis". A line naming no batch is not
 * a line that trivially satisfies that rule; it is a line the rule cannot be evaluated for at all.
 *
 * v001 left it out, and the consequence was worse than a missing report: `batchNumbersOf` skips a
 * good with no batch, so the line was invisible to certificate matching too, and a shipment whose
 * lot nobody had confirmed was received as though its paperwork were complete.
 */
