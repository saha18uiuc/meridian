...
export const REQUIRED_PATHS: readonly string[] = [
  ...
  'examples/inbound-import-receiving/evals/case-18.json',
  ...
  'examples/inbound-import-receiving/fixtures/expected/case-18.expected.json',
...
export const REQUIRED_EVAL_CASES = 18;
...
export const DECLARED_ADDITIONS: Readonly<Record<string, string>> = {
  ...
  'packages/ops/src/intake/extract-vendor-key.ts':
    'the vendor renewal deployment\'s correlation rule; correlation only looked generic while there was one deployment, because "the business key" and "a container number or an air waybill" were the same sentence — each deployment now names its own extractor in deployments.ts',
